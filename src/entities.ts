...
import { map, take, flatMap, tap, switchMap } from "rxjs/operators";
...
const appExamples$ = (appId: string): Observable<Example[]> => {
  ...
  return intents.run(c => c.find({ appId }).toArray()).pipe(
    ...
    flatMap<string[], any[]>(ids =>
      examples.run<any[]>(c => c.find({ intentId: { $in: ids } }).toArray())
    ),
    map<any[], Example[]>(docs => docs.map(doc => new Example(doc)))
  );
};

export const appEntities$ = (intentId: string): Observable<any[]> => {
  let intents = new Collection(INTENT_COLLECTION);
  ...
  server.post(
    ...
    (request: restify.Request, response: restify.Response) => {
      withJSON(request, response, json => {
        ...
        appExamples$(newEntity.appId)
          .pipe(
            map<any[], Example[]>(exs =>
              exs.reduce<Example[]>((updts, ex) => {
                let exWithEntities = withEntities(
                  newEntity.value,
                  newEntity.synonyms,
                  ex
                );
                return ex.entities.length !== exWithEntities.entities.length
                  ? updts.concat(exWithEntities)
                  : updts;
              }, [])
            ),
          ...
          .subscribe(
            ety => response.send(201, ety),
            err => response.send(400, mongoError(err))
          );
      });
    }
  );
